...
export function TourDeleteButton({ tourId, tourName }: TourDeleteButtonProps) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (!data.success) {
        throw new Error(data.error || 'Erro ao deletar tour');
      }
      ...
      toast.error(error.message || 'Erro ao deletar tour');
    } finally {
      setIsDeleting(false);
    }
  };
  ...
  return (
    <>
      ...
      <ConfirmDialog
        ...
        title="Deletar tour?"
        description={`Tem certeza que deseja deletar "${tourName}"? Esta ação não pode ser desfeita.`}
        variant="destructive"
